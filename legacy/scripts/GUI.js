...
class GUI {
  constructor() {
    ...
    this.lights
      .add(this.params, "pointLightPower", 0.0, 3.0)
      .onChange(function (value) {
        for (var i = 0; i < app.light.pointLights.length; i++) {
          app.light.pointLights[i].intensity = value > 0.2 ? value : 0;

          app.light.pointLights[i].bulb.material.emissiveIntensity =
            value > 0.2 ? value + 0.2 : 0;
          app.light.pointLights[i].bulb.material.opacity =
            value > 0.2 ? 1 : 0.5;
        }
      });
    this.lights
      .add(this.params, "directionalLightPower", 0.0, 3.0)
      .onChange(function (value) {
        app.light.directionalLight.intensity = value > 0.2 ? value : 0;

        app.light.directionalLight.bulb.material.emissiveIntensity =
          value > 0.2 ? value + 0.2 : 0;
        app.light.directionalLight.bulb.material.opacity =
          value > 0.2 ? 1 : 0.5;
      });
    ...
    this.objects
      .add(this.params, "shape", Shape.getShapes())
      .name("basic shape")
      .onChange(function (value) {
    ...
    this.material.phong
      .add(this.params, "shininess", 1, 500.0)
      .onChange(function (value) {
    ...
    this.material.PBR.add(this.params, "roughness", 0, 1.0).onChange(function (
      value
    ) {
      var mesh;
    ...
    this.material.PBR.add(this.params, "metalness", 0, 1.0).onChange(function (
      value
    ) {
      var mesh;
      for (var j = 0; j < app.meshes.length; j++) {
    ...
    this.material
      .add(this.params, "envMap", 0, 1000.0)
      ...
      .onChange(function (value) {
        ...
        for (var j = 0; j < app.meshes.length; j++) {
          ...
          mesh.material.defaultAttributeValues.uv = new Float32Array(
    ...
    this.material
      .add(this.params, "normalMap", 0, 1000.0)
      .name("Normal Map")
      .onChange(function (value) {
        var mesh, materialType;
        for (var j = 0; j < app.meshes.length; j++) {
    ...
    this.material
      .add(this.params, "bumpScaleX", -1.0, 1.0)
      .name("bump scale X")
      .onChange(function (value) {
        var mesh, bumpScaleY;
        for (var j = 0; j < app.meshes.length; j++) {
    ...
    this.material
      .add(this.params, "bumpScaleY", -1.0, 1.0)
      .name("bump scale Y")
      .onChange(function (value) {
        var mesh, bumpScaleX;
        for (var j = 0; j < app.meshes.length; j++) {
    ...
    this.textures
      .add(this.params, "skyBox", app.textures.getSkyboxKeys())
      .onChange(function (value) {
    ...
    this.textures
      .add(this.params, "texture", app.textures.getTextureKeys())
      .onChange(function (value) {
        var mesh;
        for (var j = 0; j < app.meshes.length; j++) {
          mesh = app.meshes[j];
          ...
          mesh.material.setTexturesCenter(
            app.gui.params.texOriginY,
            app.gui.params.texOriginY
          );
          mesh.material.applyRepeat(
            app.gui.params.repeatU,
            app.gui.params.repeatV
          );
    ...
    this.textures
      .add(this.params, "texOriginX", 0, 1)
      .name("origin X")
      .onChange(function (value) {
        var mesh;
        for (var j = 0; j < app.meshes.length; j++) {
          mesh = app.meshes[j];
          mesh.material.setTexturesCenter(
            app.gui.params.texOriginX,
            app.gui.params.texOriginY
          );
          mesh.material.setTexturesRotation(app.gui.params.texRotation);
          mesh.material.applyOffset(
            app.gui.params.offsetU,
            app.gui.params.offsetV,
            mesh
          );
          mesh.material.updateUvs(mesh);
          mesh.material.needsUpdate = true;
    ...
    this.textures
      .add(this.params, "texOriginY", 0, 1)
      .name("origin Y")
      .onChange(function (value) {
        var mesh;
        for (var j = 0; j < app.meshes.length; j++) {
    ...
    this.textures
      .add(this.params, "texRotation", 0, 360)
      .name("rotation")
      .onChange(function (value) {
        var mesh;
        for (var j = 0; j < app.meshes.length; j++) {
    ...
    this.view
      .add(this.minMaxGUIHelper, "min", 0.1, 10000, 0.1)
      .name("near")
      .onChange(this.updateCamera);
    this.view
      .add(this.minMaxGUIHelper, "max", 0.1, 25000, 0.1)
      .name("far")
      .onChange(this.updateCamera);
  }
  updateCamera() {
