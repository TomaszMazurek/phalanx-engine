    ...
    async populateTextureMap(){
        ...
        for (i = 0; i < keyArray.length; i++) {
            ...
            var texturePromise = new Promise(resolve => {
                new THREE.TextureLoader().load( self.textureMap[key][0] + "Base_Color.jpg", resolve);
            });
        ...
        for (i = 0; i < keyArray.length; i++) {
            var key = keyArray[i];
            var texturePromise = new Promise(resolve => {
                new THREE.TextureLoader().load( self.textureMap[key][0] + "Normal.jpg", resolve);
            });
            texture = await texturePromise;
        ...
        for (i = 0; i < keyArray.length; i++) {
            var key = keyArray[i];
            var texturePromise = new Promise(resolve => {
                new THREE.TextureLoader().load( self.textureMap[key][0] + "Ambient_Occlusion.jpg", resolve);
            });
            texture = await texturePromise;
            this.textureMap[keyArray[i]][6] = texture;
            this.textureMap[keyArray[i]][6].name = key+"_roughness";
...
class Textures {
    ...
    async populateTextureMap(){
        var i, key, texture,texturePromise,
            self = this,
            keyArray = Object.keys(self.textureMap);
